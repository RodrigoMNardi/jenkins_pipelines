                    ...
                    stage('03 - Create DB') {
                        ...
                        steps {
                            sh '''
                              ...
                              sed -i "s/githubruns-test/githubruns-test-$POSTGRES_PORT/g" config/database.yml
                              cp db/schema.rb db/schema.rb.bak
                              # Encerra conexões ativas antes do reset
                              psql -h 172.17.0.1 -U postgres -d postgres -c "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname='githubruns-test-$POSTGRES_PORT' AND pid <> pg_backend_pid();"
